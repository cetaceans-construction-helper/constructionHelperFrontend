...
### 2.1 동일 템플릿 사용 (Top/Sub 상호 배타)

feature 구조는 아래 두 모드 중 하나를 선택한다.

모드 A (Top 단일):

```txt
src/features/<top-feature>/
  ui/
  view-model/
  use-cases/
  model/
...
```txt
src/features/<top-feature>/<sub-feature>/
  ui/
  view-model/
  use-cases/
  model/
...
규칙:
- `MUST`: 새 feature 생성 시 선택한 모드의 템플릿을 그대로 사용한다.
- `MUST`: 파일이 적어도 폴더 구조는 먼저 만든다.
- `MUST`: sub feature를 채택한 top feature에는 top 템플릿을 두지 않는다.
- `MUST NOT`: top 템플릿과 sub 템플릿을 동시에 공존시키지 않는다.
...
```ts
import { createDeliveryUseCase } from '@/features/material/order/public'
```

금지 예시:

```ts
import { createDeliveryUseCase } from '@/features/material/order/use-cases/createDeliveryUseCase'
```
...
```ts
import type { Order } from '@/features/material/order/model/order'
```
...
- [ ] sub feature를 쓰는 top feature에는 top 템플릿이 없다.

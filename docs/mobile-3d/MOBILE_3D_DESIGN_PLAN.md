...
- [x] [검증] 상단 패널이 실제 3D 모델로 채워짐
- [x] [검증] 다양한 모바일 해상도에서 상단 영역이 우선 확보됨
- [x] [검증] 전체 페이지 스크롤 없이 3D 조작 가능
- [x] [리뷰] 상단 패널 조작 UX(터치 반응/가시성) 확인
- [x] [완료기준] 모바일 상단 패널에서 3D 모델 실사용 가능

### 9.5 Gate 4 - 하단 패널 미니맵 구현 (층별 지도)
- [x] [작업] 모델 기반 층별 미니맵 데이터 생성 로직 구현
- [x] [작업] `MiniMap2d` 컴포넌트로 하단 패널 기본 화면 채우기
- [x] [작업] 층 탭 + 부재 마커(점/사각형) 렌더링
- [x] [작업] 미니맵 탭 시 부재 선택 + 카메라 이동 연결
- [x] [작업] 3D 선택과 미니맵 하이라이트 양방향 동기화
- [x] [작업] 파일럿용 층 기준 높이 하드코딩 적용 (1F=158, 2F=1689, 3F=3221, 4F=4753)
- [x] [작업] 파일럿용 눈높이 오프셋 하드코딩 적용 (`baseY + 600`)
- [x] [검증] 하단 패널이 미니맵으로 채워져 위치 선택 가능
- [x] [검증] 층 전환 시 마커 데이터가 정확히 갱신
- [x] [리뷰] 위치 직관성(층/구역 탐색성) 검토
- [x] [완료기준] 하단 패널에서 층별 미니맵으로 위치 선택 가능

### 9.6 Gate 5 - 세부작업 패널 + 하단 전환 로직
- [x] [작업] `selectedObject3d == null`일 때 하단에 미니맵 표시
- [x] [작업] `selectedObject3d != null`일 때 하단을 `세부작업 패널`로 전환
- [x] [작업] 세부작업 패널 상단에 선택 부재 기본 정보(ID, 층, 구역/섹션, 부재코드) 표시
- [x] [작업] 부재 선택 시 `taskApi.getTaskList(object3dId)` 호출로 세부작업 목록 로드
- [x] [작업] 세부작업 목록에 공종/세부공종/구분/계획수량 필드를 PC와 동일한 의미로 표시
- [x] [작업] 세부작업 목록에서 항목 선택/해제(단일 선택) 가능하도록 UI 제공
- [x] [작업] `선택 해제` 또는 `미니맵으로 돌아가기` 액션 시 하단을 미니맵으로 복귀
- [x] [작업] `작업 완료/추가 조치 필요/메모` 액션 UI는 Gate 5 범위에서 제외하고 후속 게이트로 이관
- [ ] [검증] 빠른 연속 탭에서도 미니맵↔세부작업 전환 상태 꼬임 없음
- [ ] [검증] 로딩/빈 목록/에러 상태가 패널에서 명확히 구분되어 표시됨
- [ ] [검증] 3D 선택 상태와 하단 패널 데이터가 항상 동일 부재 기준으로 동기화됨
- [ ] [리뷰] 세부작업 데이터 필드/액션 흐름이 PC(`Schedule3dPage`)와 일관되는지 확인
- [ ] [완료기준] 모바일에서 `부재 선택 -> 세부작업 확인 -> 복귀` 흐름이 안정적으로 동작
...
- [ ] PR-5: 세부작업 패널 + 전환 + 세부작업 선택 (Gate 5)
- [ ] PR-6: 미니맵 고도화 + 성능 튜닝 + QA (Gate 6~8)

## 12. TODO (하드코딩 제거)
- [ ] 현재 파일럿에서 사용 중인 층 기준 높이 하드코딩(1F=158, 2F=1689, 3F=3221, 4F=4753)을 API 기반 값으로 교체한다.
- [ ] 현재 파일럿 눈높이 오프셋(`+600`)을 API/설정 기반으로 교체한다.
- [ ] 눈높이 오프셋은 최종 `1600mm` 기준으로 전환한다.
- [ ] 하드코딩 제거 후 `#/m/helper/schedule/3d`에서 층 이동/시점 이동 회귀 검증을 수행한다.

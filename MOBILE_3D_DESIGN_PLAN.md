...
### 9.3 Gate 2 - 3D 공용 코어 추출(행동 불변)
- [ ] [작업] 모델 로딩/선택/API 호출 로직을 공용 composable로 추출
- [ ] [작업] PC 뷰가 공용 composable을 사용하도록 교체
- [ ] [작업] 모바일 뷰도 같은 composable 연결
- [ ] [작업] 상태 타입 정의(`selectedObject3d`, 선택 ID 목록 등) 통합
- [ ] [검증] PC에서 선택/회전/줌/일보 연동 기존과 동일
- [ ] [검증] 모바일 스켈레톤에서 모델 로딩 및 단일 선택 가능
- [ ] [검증] 콘솔 에러/메모리 누수 징후 없음
- [ ] [리뷰] “추출 전/후 동작 동일” 비교 근거 제시
- [ ] [완료기준] UI 분기 없이 코어 재사용 기반 확보

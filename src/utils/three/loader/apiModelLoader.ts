 ...
 */
function createGeometryFromApiData(model: Model3dm): THREE.BufferGeometry {
  const geometry = new THREE.BufferGeometry()
  const { data } = model.geometry
 ...
 */
export function createMeshFromApiData(model: Model3dm): THREE.Mesh {
  const geometry = createGeometryFromApiData(model)

  const { r, g, b } = model.layerColor
 ...
 */
export function createModelFromApiData(models: Model3dm[]): THREE.Group {

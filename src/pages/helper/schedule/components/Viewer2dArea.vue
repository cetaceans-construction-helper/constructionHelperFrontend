...
const startGroupDrag = (index: number, event: MouseEvent) => {
  ...
  workNodes.forEach((n) => {
    ...
    const groupKey =
      box.level === 'workType' ? work.workType || '미분류' : work.subWorkType || '미분류'
  ...
  <div class="h-full">
    ...
    >
      ...
      >
              ...
              <template v-if="showWorkTypeGroup">
                <template v-for="(box, i) in groupBoxes" :key="`group-wt-svg-${i}`">
                  <rect
                    ...
                    :x="box.x"
                    :y="box.y - HEADER_HEIGHT / viewport.zoom"
                    :width="box.width"
                    :height="box.height"
                    :fill="box.fillColor"
        ...
        >
          ...
          >
            ...
            <!-- 완료일 (읽기전용) -->
            <p class="text-[11px] text-muted-foreground">
              완료일: <span class="font-medium text-foreground">{{ tooltip.completionDate }}</span>
            </p>
      ...
      >
        <label class="flex items-center gap-1.5 cursor-pointer">
          <Checkbox
            ...
            class="h-3.5 w-3.5"
            @update:model-value="
              showWorkTypeGroup = !!$event;
              if ($event) showSubWorkTypeGroup = false;
            "
          />
        ...
        <label class="flex items-center gap-1.5 cursor-pointer">
          <Checkbox
            ...
            class="h-3.5 w-3.5"
            @update:model-value="
              showSubWorkTypeGroup = !!$event;
              if ($event) showWorkTypeGroup = false;
            "
          />
      ...
      <div class="absolute bottom-4 left-4 z-30 flex items-center gap-3">
        ...
        >
          <kbd
            class="px-1.5 py-0.5 rounded border text-xs font-mono"
